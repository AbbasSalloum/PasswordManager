            ...
            if (DecryptBtn == e.getSource()) {
                try {
                    String text = JOptionPane.showInputDialog("Enter the text to decrypt"); // getting the encrypted
                                                                                            // text
                    String secretKey = JOptionPane.showInputDialog("Enter the secret key"); // getting the secret
                                                                                            // key
                    if (text.length() > 0 && secretKey.length() > 0) // checking if the text and secret key is not
                                                                     // empty
                    {
                        // password generator class reference
            ...
            if (PassSearchBtn == e.getSource()) {
                try {
                    String acc_name = JOptionPane.showInputDialog("Enter your Account Name"); // getting the account
                                                                                              // name
                    if (!acc_name.isEmpty()) { // checking if the account name is not empty
                        Object pass = data.get_Acc(acc_name.toLowerCase()); // getting the password of the account
                                                                            // name
                        if (pass != null) { // checking if the password is not null
                            ...
                            JOptionPane.showMessageDialog(conn1, new JScrollPane(searchPassArea),
